...
//{
    public class ProjectileController : MonoBehaviour
    {
        ...
        private void OnCollisionEnter(Collision collision)
        {
            ...
            if((collision.collider.gameObject.layer == 7) && collision.collider.tag != "LastDefender")
            {
                collision.collider.GetComponentInParent<TowerRemainingParts>().towerPieces.Clear();
                
                GameManager.Instance.AudioManager.SoundToPlay(GameManager.Instance.AudioManager.towerBaseCompletelyDestruction);
                
                collision.collider.GetComponentInChildren<ParticleSystem>().Play();
                
                Destroy(collision.collider.gameObject, 0.75f);
            }
